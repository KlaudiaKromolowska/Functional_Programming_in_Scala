...
case class Fork(
  ...
  val massX: Float ={
    ...
    else List(nw,ne,sw,se).foldLeft(0f){
      case (sum,q) => sum+q.mass*q.massX
    }/mass
  }
  val massY: Float = {
    if mass==0 then centerY
    else List(nw,ne,sw,se).foldLeft(0f){
      case (sum,q) => sum+q.mass*q.massY
    }/mass
  }
    ...
    def traverse(quad: Quad): Unit = (quad: Quad) match
      ...
      case Fork(nw, ne, sw, se) =>
        ...
        if quad.size/distance(x,y,quad.massX, quad.massY)<theta
          then addForce(quad.mass, quad.massX, quad.massY)
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix =
    apply(
      ((min(max(b.x, boundaries.minX), boundaries.maxX) - boundaries.minX)/sectorSize).toInt,
      ((min(max(b.y, boundaries.minY), boundaries.maxY) - boundaries.minY)/sectorSize).toInt
    )+=b
    this
