...
object LineOfSight extends LineOfSightInterface:
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit =
    input.zipWithIndex.foreach{
      ...
      case (xs,i)=> output(i) = Math.max(output(i-1), xs/i)
    }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float =
    (from until until).foldLeft(0f)(
      (currentMaximum, i) => List(input(i)/i, currentMaximum).max
    )
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    threshold: Int): Tree =
    if(end-from <= threshold)  Tree.Leaf(from, end, upsweepSequential(input, from, end))
    else {
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    startingAngle: Float, from: Int, until: Int): Unit = //check
    (from until until).foldLeft(0f)(
      (startingAngle, i) => Math.max(input(i)/i, startingAngle)
    )
