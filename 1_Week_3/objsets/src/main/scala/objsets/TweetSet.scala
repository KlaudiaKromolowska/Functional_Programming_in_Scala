...
<<<<<<< HEAD
      ...
      val temp = mostRetweeted
      new Cons(temp, remove(temp).descendingByRetweet)
    }
=======
...
>>>>>>> 933508d (Week_3)
  ...
  override def mostRetweeted: Tweet = {
    def moreRetweets(t1:Tweet, t2:Tweet):Tweet = if t1.retweets>t2.retweets then t1 else t2
    if (left.isEmpty && right.isEmpty) elem
    else if (right.isEmpty) moreRetweets(left.mostRetweeted, elem)
    else if (left.isEmpty) moreRetweets(right.mostRetweeted, elem)
<<<<<<< HEAD
    else moreRetweets(elem, moreRetweets(left.mostRetweeted, right.mostRetweeted))
=======
