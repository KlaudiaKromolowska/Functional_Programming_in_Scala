...
object TimeUsage extends TimeUsageInterface:
  ...
  def timeUsageByLifePeriod(): Unit =
    val (columns, initDf) = read("src/main/resources/atussum.csv")
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) =
    val primaryNeeds = columnNames.filter (c => c.matches("(t01|t03|t11|t1801|t1803).*")).map(col)
    val workingActivities = columnNames.filter(c => c.matches("(t05|t1805).*")).map(col)
    val otherActivities = columnNames.filter(name => !primaryNeeds.contains(col(name)) && !workingActivities.contains(col(name)))
      .filter(c => c.matches("(t02|t04|t06|t07|t08|t09|t10|t12|t13|t14|t15|t16|t18).*")).map(col)
  ...
  def timeUsageSummary(
    ...
    def minutesToHours(columns: Column*) = columns.reduce(_+_).divide(60)
    val primaryNeedsProjection: Column = minutesToHours(primaryNeedsColumns: _*).as("primaryNeeds")
    val workProjection: Column = minutesToHours(workColumns: _*).as("work")
    val otherProjection: Column = minutesToHours(otherColumns: _*).as("other")
    df
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] =
    summed
      ...
      .map { (x, p, w, o) =>
        TimeUsageRow(x._1, x._2, x._3, p, w, o)
