...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.map((block:Block, move:Move) => (block, move::history)).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filter((block:Block, _) => !explored.contains(block))
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
    val next = for{
      (block: Block, history:List[Move]) <- initial
      (neighbourBlock, moves) <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
      if !explored.contains(neighbourBlock)
    } yield (neighbourBlock,moves)
    initial #::: from(next, explored ++ next.map(_._1)) //#::: concatenate two lazy lists
  }
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter((block:Block, _)=> done(block))
