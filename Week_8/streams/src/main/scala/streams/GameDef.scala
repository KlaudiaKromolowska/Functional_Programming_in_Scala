...
trait GameDef:
  ...
  case class Block(b1: Pos, b2: Pos):
     ...
     */
    def neighbors: List[(Block, Move)] = List((up,Move.Up), (down, Move.Down), (left,Move.Left), (right, Move.Right))

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter((block:Block, _) => block.isLegal)
