...
trait EncoderInstances:
  ...
  given Encoder[String] = Encoder.fromFunction(s => Json.Str(s))
...
trait DecoderInstances:
  ...
  given Decoder[Int] = Decoder.fromPartialFunction{
    case Json.Num(x) if x.isValidInt => x.intValue()
  }

  /** A decoder for `String` values */
  given Decoder[String] = Decoder.fromPartialFunction{
    case Json.Str(s) => s
  }

  /** A decoder for `Boolean` values */
  given Decoder[Boolean] = Decoder.fromPartialFunction{
    case Json.Bool(b) => b
  }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(list) =>
        val decoded = list.map(x => decoder.decode(x))
        if decoded.forall(_.isDefined) then Some(decoded.map(x=>x.get)) else None
      case _ => None
    }
  ...
  /**
    ...
    */
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] = //check this
    Decoder.fromFunction{
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts]=
    Decoder.field[List[Person]]("people")
      .transform[Contacts](x => Contacts(x))
